...
export function KareebuTopPicks({
  ...
  // Careem-like proportions:
  // roughly 2.25 product cards visible on a normal phone.
  const cardWidth = useMemo(() => {
    const usable = Math.max(320, width);
    return Math.round(Math.min(174, Math.max(146, (usable - SIDE_PADDING * 2 - GAP * 1.3) / 2.28)));
  }, [width]);
  ...
  const onMomentumEnd = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    const next = Math.max(
      0,
      Math.min(GROCERY_PICKS.length - 1, Math.round(x / snapInterval))
    );
    setActiveIndex(next);
  };
