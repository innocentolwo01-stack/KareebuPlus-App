...
import { BrandedSuccessState } from '../components/BrandedState';
import { CategoryLandingBanner } from '../components/CategoryLandingBanner';
import { categoryBannerAssets } from '../assets/categoryBannerAssets';
...
const sendCategories:Array<{id:string;banner:typeof categoryBannerAssets.send.sendParcel;parcelType:ParcelDraft['parcelType'];weight:number}>=[
  {id:'send-parcel',banner:categoryBannerAssets.send.sendParcel,parcelType:'Small parcel',weight:3},
  {id:'send-documents',banner:categoryBannerAssets.send.sendDocuments,parcelType:'Documents',weight:1},
  {id:'deliver-gift',banner:categoryBannerAssets.send.deliverGift,parcelType:'Small parcel',weight:3},
  {id:'business-delivery',banner:categoryBannerAssets.send.businessDelivery,parcelType:'Medium parcel',weight:8},
];
...
export function ParcelStartScreen({ data, actions }: { data: ParcelScreenData; actions: ParcelScreenActions }) {
  ...
  return <ScreenShell><Header title="Send parcel" onBack={() => actions.go('home')}/><ScrollView ref={scrollRef} style={styles.flex} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
    <CategoryLandingBanner banner={mainCategoryBanner('send')} accessibilityLabel="Send Something with Kareebu. Choose a delivery type" accessibilityHint="Moves to delivery type selection" onPress={()=>scrollRef.current?.scrollTo({y:220,animated:true})}/>
    ...
    <SectionTitle title="Choose a delivery type"/>
    <ScrollView horizontal nestedScrollEnabled showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bannerRail}>{sendCategories.map(item=><CategoryLandingBanner key={item.id} banner={item.banner} style={styles.sendCategoryBanner} accessibilityLabel={`${item.banner.title}. Select this delivery type`} accessibilityHint="Selects this delivery type before entering the booking details" onPress={()=>actions.updateDraft({parcelType:item.parcelType,weightKg:item.weight})}/>)}</ScrollView>
    <Text style={styles.title}>Parcel size</Text><Text style={styles.subtitle}>Choose the closest size, then confirm pickup and destination.</Text>
    ...
    <PrimaryButton disabled={!data.draft.pickupAddress.trim()||!data.draft.dropoffAddress.trim()} label="Add recipient details" onPress={()=>actions.go('parcelContacts')}/>
...
export function ParcelSuccessScreen({ data, actions }: { data: ParcelScreenData; actions: ParcelScreenActions }) {
  ...
  return <ScreenShell><View style={styles.success}><BrandedSuccessState art={require('../../assets/kareebu-plus/lifestyle-cutouts/service-send.png')} title="Courier request confirmed" body={`Parcel #${order.id} · ${order.pickupAddress} to ${order.dropoffAddress} · ${formatMoney(data.country,order.fee)}`} primaryLabel="Track parcel" onPrimary={()=>actions.go('parcelTracking')} secondaryLabel="Send another parcel" onSecondary={()=>actions.go('parcel')}/></View></ScreenShell>;
}
