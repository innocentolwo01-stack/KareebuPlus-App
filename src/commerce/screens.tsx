...
import { commerceProductFor, commerceProductsFor } from './catalog';
  ...
  return (
    <ScreenShell>
      ...
      <ScrollView style={styles.flex} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.quantityRow}><Pressable onPress={() => setQuantity((value) => Math.max(1, value - 1))} style={styles.qtyButton}><Feather name="minus" size={20}/></Pressable><Text style={styles.qtyValue}>{quantity}</Text><Pressable onPress={() => setQuantity((value) => Math.min(99, value + 1))} style={styles.qtyButton}><Feather name="plus" size={20}/></Pressable></View>
        <PrimaryButton disabled={!prescriptionAttached} label={`Add ${quantity} · ${formatMoney(data.country, unitPrice * quantity)}`} onPress={() => { actions.addLine({ id: lineId, storeId: store.id, productId: product.id, quantity, unitPrice, variantId: selectedVariant.id, variantLabel: selectedVariant.label, note }); actions.go('commerceCart'); }}/>
      </ScrollView>
    </ScreenShell>
  );
}
...
export function CommerceCheckoutScreen({ data, actions }: { data: CommerceScreenData; actions: CommerceScreenActions }) {
  ...
  const subtotal = lines.reduce((sum, line) => sum + line.unitPrice * line.quantity, 0);
  const itemCount = lines.reduce((sum, line) => sum + line.quantity, 0);
  const deliveryFee = data.checkout.fulfillment === 'pickup' ? 0 : store.deliveryFee;
...
const styles = StyleSheet.create({
  flex:{flex:1}, scroll:{paddingHorizontal:20,paddingTop:12,paddingBottom:34,gap:18},
  ...
  productName:{fontFamily:FONT.bold,fontSize:27,lineHeight:32,fontWeight:'900',color:COLORS.black}, productDetail:{...TYPE.body,color:COLORS.muted,lineHeight:21}, productPrice:{fontFamily:FONT.bold,fontSize:22,fontWeight:'900'},
  ...
  noteInput:{minHeight:88,borderWidth:1,borderColor:COLORS.line,borderRadius:16,padding:14,textAlignVertical:'top',fontFamily:FONT.regular,fontSize:15,color:COLORS.black}, quantityRow:{height:52,alignSelf:'center',flexDirection:'row',alignItems:'center',gap:18}, qtyButton:{width:40,height:40,borderRadius:20,borderWidth:1,borderColor:COLORS.line,alignItems:'center',justifyContent:'center'}, qtyValue:{fontFamily:FONT.bold,fontSize:18,fontWeight:'900'},
