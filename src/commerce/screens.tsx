...
import type { CommerceRouteContext } from './context';
...
export function CommerceProductScreen({ data, actions }: { data: CommerceScreenData; actions: CommerceScreenActions }) {
  ...
  const productVisual = commerceProductVisual({ id: product.id, name: product.name, category: product.category, subcategory: product.subcategory, detail: product.detail, brand: product.brand, imageKey: product.metadata.imageKey, storeCategory: store.category });
  const liveStore = Boolean(store.contentTrust?.liveAvailability);

  return (
    <ScreenShell>
      <Header title={store.name} onBack={actions.back} />
      <ScrollView style={styles.flex} contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[styles.productVisual,{backgroundColor:productVisual.background}]}>{productVisual.image?<Image source={productVisual.image} style={styles.productHeroImage} resizeMode="contain"/>:<View style={{width:148,height:148,borderRadius:24,backgroundColor:'#F4F1EB'}}/>}{product.badge ? <View style={styles.badge}><Text style={styles.badgeText}>{product.badge}</Text></View> : null}<View style={styles.productSellerBadge}><SellerLogo name={store.name}/></View></View>
        <View style={styles.productSellerRow}><View style={styles.productSellerLogo}><SellerLogo name={store.name}/></View><View style={styles.flex}><Text style={styles.productSellerName}>{store.name}</Text><Text style={styles.productSellerMeta}>{store.category} · Seller catalogue</Text></View></View>
        ...
        <View style={styles.ratingStockRow}><Text style={styles.ratingStockText}>{liveStore && productMetadata.averageRating ? `★ ${productMetadata.averageRating.toFixed(1)} · ${productMetadata.ratingCount} reviews` : 'Details confirmed before checkout'}</Text><Text style={styles.ratingStockText}>{liveStore && productMetadata.stock !== undefined ? `${productMetadata.stock} in stock` : 'Check current availability'}</Text></View>
  ...
  return (
    <ScreenShell>
      ...
      <ScrollView style={styles.flex} contentContainerStyle={styles.scroll}>
        <View><Text style={styles.storeName}>{store.name}</Text><Text style={styles.storeMeta}>{itemCount} item{itemCount === 1 ? '' : 's'} · {store.contentTrust?.liveAvailability ? store.eta : 'Check availability'}</Text></View>
        {lines.length ? lines.map((line) => {
          const product = products.find((item) => item.id === line.productId);
          if (!product) return null;
          const visual=commerceProductVisual({id:product.id,name:product.name,category:product.category,subcategory:product.subcategory,detail:product.detail,brand:product.brand,imageKey:product.metadata.imageKey,storeCategory:store.category});
          return <RoundedCard key={line.id} style={styles.cartLine}><View style={[styles.cartIcon,{backgroundColor:visual.background}]}>{visual.image?<Image source={visual.image} style={styles.cartProductImage} resizeMode="contain"/>:<View style={{width:48,height:48,borderRadius:12,backgroundColor:'#F4F1EB'}}/>}</View><View style={styles.flex}><Text style={styles.optionTitle}>{product.name}</Text><Text style={styles.optionMeta}>{line.variantLabel ?? product.detail}{line.note ? ` · ${line.note}` : ''}</Text><Text style={styles.linePrice}>{formatMoney(data.country, line.unitPrice * line.quantity)}</Text></View><View style={styles.lineQty}><Pressable onPress={() => actions.setLineQuantity(line.id, line.quantity - 1)}><Feather name="minus" size={16}/></Pressable><Text style={styles.lineQtyText}>{line.quantity}</Text><Pressable onPress={() => actions.setLineQuantity(line.id, line.quantity + 1)}><Feather name="plus" size={16}/></Pressable></View></RoundedCard>;
...
export function CommerceCheckoutScreen({ data, actions }: { data: CommerceScreenData; actions: CommerceScreenActions }) {
  ...
  const applyCoupon = () => {
    actions.updateCheckout({ couponCode: null });
    if (coupon.trim()) Alert.alert('No configured coupon', 'This build will only apply coupon benefits supplied by configured campaign data.');
  };
...
const styles = StyleSheet.create({
  ...
  productVisual:{height:238,borderRadius:24,backgroundColor:'#F7F4EE',alignItems:'center',justifyContent:'center',position:'relative',overflow:'hidden'},productHeroImage:{width:'82%',height:'82%'},productSellerBadge:{position:'absolute',right:12,top:12,width:78,height:48,borderRadius:13,overflow:'hidden'},productSellerRow:{minHeight:58,borderRadius:16,borderWidth:1,borderColor:COLORS.line,backgroundColor:COLORS.white,padding:9,flexDirection:'row',alignItems:'center',gap:10},productSellerLogo:{width:54,height:40,borderRadius:11,overflow:'hidden'},productSellerName:{fontFamily:FONT.bold,fontSize:13,fontWeight:'900',color:COLORS.black},productSellerMeta:{...TYPE.caption,color:COLORS.muted,marginTop:2}, badge:{position:'absolute',left:14,top:14,borderRadius:12,backgroundColor:COLORS.yellow,paddingHorizontal:10,paddingVertical:6}, badgeText:{fontFamily:FONT.bold,fontSize:11,fontWeight:'900'},
